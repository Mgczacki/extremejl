...
C Creates "strings" and in some cases pre-emptively writes constants (what you would do with data)
      CHARACTER*8  ATNAM
...
C Creates n-dimensional arrays
...
C Defines labels for statements (can be called with this label).
...
C Julia code currently covers 0, use case is 1. Others would be interesting to get
...
C Calls functions to check that arguments are properly formed
      CALL MAKNAME(1,WFN,ILEN,FWFN)
      IF (ILEN .EQ. 0) STOP 'usage:  extreme wfnfile '
      CALL MAKNAME(1,WLP,ILEN,FWLP)
      IF (ILEN .EQ. 0) STOP 'usage:  extreme wfnfile '
C Opens file for reading, and for writing
...
C
      CALL RDWFN
C Reads wfn to memory
C
      WRITE (IOUT,1010) WFNTTL
      WRITE (IOUT,1000)
C Reads the Job title
...
C
      ...
      WRITE(IOUT,*) 
C Select where to search
30    WRITE (IOUT,1120)
      READ (INPT,*) IST
C
      IF (IST .EQ. 0) THEN
C OPTION 0: Calculates \rho at a single point
        write(iout,1121)
        Read(Inpt,*) nucarb
c Rho at Arbitrary coordinates
        If(nucarb.eq.0)Then
        WRITE (IOUT,1130)
c Reads coordinates
        READ (INPT,*) XYZ(1),XYZ(2),XYZ(3)
c If verbose, print starting coordinates for Newton Raphson
...
c Sets variables
        ...
        J2 = 0
        Inuc=0
c Rho at Nucleus coordinates
        ElseIf(nucarb.eq.1)Then
c Sets variables (note difference)
        J2=0
        Inuc=1
c Nucleus number at critical point
        write(iout,1122)
        Read(Inpt,*)J1
c Reads coordinates of nucleus
        XYZ(1)=CO(IXC+J1)
        XYZ(2)=CO(IYC+J1)
        XYZ(3)=CO(IZC+J1)
c If verbose, writes to console
        If(Iprint.eq.1)WRITE (IOUT,1141) XYZ(1),XYZ(2),XYZ(3)
        WRITE (IOUT,*)
        WRITE (IWLP,*)
        WRITE (IWLP,1141) XYZ(1),XYZ(2),XYZ(3)
        Endif
c Sets more variables, calls Netwon-Raphson
        IWHOLE=0
        Ifail=0
...
C Init CO(IPSI) as 0
      ...
      CO(IPSI+J)=ZERO
10    CONTINUE
C For every primitive, calculate \psi_\mu
      DO 20 I=1,NPRIMS
      K=IC(ICENT+I)
      ENT=CO(IE+I)
C These are deltas
...
C Call primeF (Iwhat is 1 because of rho)
C This calculates the value of F0 (EXPON), and its first and second derivatives
      Call PrimeF(Ent,Expon,X,Y,Z)
C Iwhich is the type of the primitive
      IWhich=IC(ITYPE+I)
C 20 possible types, initialization of A0, A*
      Call PrimeA(IWhich,Ifill,X,Y,Z)
C Generates G0 and its first/second derivatives
      Call PrimeG
C Again, generates A0, A*
      Call PrimeA(IWhich,IEmpty,X,Y,Z)
C Check = max(abs(G0), abs(Gx), abs(Gy), abs(Gz))
      Check=Dmax1(dabs(G0),dabs(Gx),dabs(Gy),dabs(Gz))
c this is probably not important, assume always true
c if check*Icofmx[primitive] > cutoff then
      If(check*CO(Icofmx+I).gt.cutoff)Then
      DO 30 J=1,NMO
C CIJ is the coefficient for this primitive, MO
      CIJ=CO(IMO+NPRIMS*(J-1)+I)
C Note: G0 is the Gaussian belonging to the primitive (\Psi_\mu)
...
40    CO(IGZZ+J)=CO(IGZZ+J)+CIJ*GZZ
      Endif
      Endif
C Rho does not enter the next Iwhats
      If(Iwhat.gt.1)Then
...
C
      RETURN
      END
C This subroutine generates the gradients for \rho, as well as its hessian
      SUBROUTINE GRDRHO(Iopt,R,VALUE,W,GRAD,H,SG)
C Iopt = 1 for rho
C R is the XYZ coordinates of a point
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
      Parameter(MaxOff=200000)
      COMMON CO(MaxOff),IC(MaxOff),NCENT,NMO,NPRIMS
...
C
     ...
     2              CO(IZZ+I)*CO(IZZ+I)
        CO(IRR+I) = DSQRT(CO(IR2+I))
C CO(IRR+I) contains the distance from R to each nuclei I now
C We update RMIN to be min(distance_i, 1000)
C MINR contains the index of nuclei with the min distance
C BANGLE is a 3 element vector containing: (R - R_minr)/(distance(R,R_i)+\epsilon)
        IF(CO(IRR+I) .LT. RMIN) THEN
          RMIN = CO(IRR+I)
          MINR = I
          BANGLE(1) = CO(IXX+I)/(CO(IRR+I)+Small)
          ...
          BANGLE(3) = CO(IZZ+I)/(CO(IRR+I)+Small)
        ELSE IF (CO(IRR+I) .GT. RMAX) THEN
C RMAX = max(0, distance_i)
          RMAX = CO(IRR+I)
        END IF
10    CONTINUE
C Init CO(IPSI) as 0
C Calculate IPSI, IG*, IG**
      CALL GAUS4
C Init W, H SG as 0.0
      DO 20 I = 1,3
        W(I) = Zero
        DO 20 J = I,3
          H(I,J) = Zero
...
C Filled in W, H, SG
      Do 60 I=1,3
        DO 60 J = I,3
          ...
          SG(J,I) = SG(I,J)
60    CONTINUE
C H and SG are symmetric matrices
70    GRAD=Zero
      DO 80  I = 1,3
      W(I) = W(I)*Two
...
C
      ...
      toalpe=toalp*expon
C toalp = -2\alpha_\mu
C toalpe = -2\alpha_\mu*exp(-alpha_mu*dist^2)
C F0 = Value of exp(-alpha_mu*dist^2)
      F0=EXPON 
C dF0/dx
      FX=toalpe*X
C dF0/dy
...
C This is the line. A0*exp(-alpha_mu*dist^2)
      G0=A0*F0
C
      GX=AX*F0+A0*FX
      GY=AY*F0+A0*FY
...
C WFNTTL = Get title of file
      READ(IWFN,101) WFNTTL
C Read Mode=GAUSSIAN, NMO = # Mol Orbitals, NPRIMS = # primitives, NCENT = # Nuclei
      READ(IWFN,102) MODE,NMO,NPRIMS,NCENT
C
C     The following are all OFFSETS, sequentially calculated.
...
C
      ...
      IGZZZZ=IGYZZZ+NMO
      ICOFMX=IGZZZZ+NMO
C Stores nuclei information in CO, ATNAM (Atom Name)
      DO 100 I = 1,NCENT
        READ (IWFN,103) ATNAM(I),J,CO(IXC+J),CO(IYC+J),CO(IZC+J),
     +                  CO(ICHARG+J)
100   CONTINUE
C IC contains center assignments and type assignments for each primitive
      READ (IWFN,104) (IC(ICENT+I),I=1,NPRIMS)
      READ (IWFN,104) (IC(ITYPE+I),I=1,NPRIMS)
C Reads exponents into CO for each primitive
      READ (IWFN,105) (CO(IE+I),I=1,NPRIMS)
C Checks that we are not working with more than 999 primitives, exits if necessary (limitation of code)
      DO 130 I = 1,NPRIMS
       IF(IC(ITYPE+I).GT.NTYPE) GOTO 999
 130  CONTINUE
C Stores OCC NO in CO(IP+I) and ORB ENERGY in CO(IEORB+I) for each MO
C Then stores MOs (num_primitives c_µp for each MO) in CO(K+J)
      DO 120 I = 1,NMO
        READ (IWFN,106) CO(IP+I),CO(IEORB+I)
        K = NPRIMS*(I-1)+IMO
        READ (IWFN,107) (CO(K+J),J=1,NPRIMS)
120   Continue
C 1 ICOFMX x NPrims, in which you get the max value for each primitive over all MOs
      Do 125 I=1,NPrims
      check=zero
      Do 126 J=1,NMO
